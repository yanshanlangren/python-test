        ...
        for item in source:
            if item["meter"] == "apisix_bandwidth":
                ...
                if tag_map["type"]:
                    ...
                    temp = item["value"]
                    item["value"] = str(int(temp) - cache_value)
            ...
            if item["meter"] == "apisix_http_overhead_count":
                tagstr = item["tags"]
                tags = tagstr.split(",")
                tag_map = {}
                for tag in tags:
                    tag_map[tag.split("=")[0]] = tag.split("=")[1]
                cache_value = 0
                if tag_map["node"] in cache["apisix_http_overhead_count"]:
                    cache_value = cache["apisix_http_overhead_count"][tag_map["node"]]
                temp = item["value"]
                item["value"] = str(int(temp) - cache_value)
                cache["apisix_http_overhead_count"][tag_map["node"]] = int(temp)
