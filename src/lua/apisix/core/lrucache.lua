...
item_release, key, version)
    --log.info("entering fetch_valid_cache...")
...
local function new_lru_fun(opts)
    ...
    return function(key, version, create_obj_fun, ...)
        --log.info("entering return func of new_lru_fun...")
        --log.info("phase:", get_phase())
        --log.info("lru_obj:", json.delay_encode(lru_obj, true))
        if not can_yield_phases[get_phase()] then
            local cache_obj = fetch_valid_cache(lru_obj, invalid_stale, item_ttl, item_release, key, version)
            --log.info("cache_obj:", json.delay_encode(cache_obj, true))
            if cache_obj then
                return cache_obj
            end
            ...
            elseif obj ~= nil then
                local cached_obj = setmetatable({ val = obj, _cache_ver = version }, lua_metatab)
                lru_obj:set(key, cached_obj, item_ttl)
            end

            return obj, err
        end
        --log.info("trying can yield phase...")
        local cache_obj = fetch_valid_cache(lru_obj, invalid_stale, item_ttl, item_release, key, version)
        --log.info("cache_obj:", json.delay_encode(cache_obj, true))
        if cache_obj then
            return cache_obj
        ...
        cache_obj = fetch_valid_cache(lru_obj, invalid_stale, item_ttl, nil, key, version)
        ...
        local obj, err = create_obj_fun(...)
        if type(obj) == 'table' then
            --log.info("obj:", json.delay_encode(obj, true))
            obj._cache_ver = version
            lru_obj:set(key, obj, item_ttl)
...
local function _plugin(plugin_name, key, version, create_obj_fun, ...)
    --log.info("entering _plugin...")
    local lru_global = global_lru_fun("/plugin/" .. plugin_name, nil, lru_new, PLUGIN_ITEMS_COUNT)
